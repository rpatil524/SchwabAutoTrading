...
class Options(object):
    ...
    def __init__(self, position_json):
        ...
        self.expiration_date = datetime.strptime(
            symbol_combined[0:6], '%y%m%d').date()
        # option type is either 'CALL' or 'PUT'
        self.option_type = OptionType.CALL if instrument.get(
            'putCall') == 'CALL' else OptionType.PUT
    ...
    def is_losing(self) -> bool:
        ...
        days_to_expiration = (self.expiration_date -
                              datetime.today().date()).days
        if days_to_expiration > 14 or self.option_type == OptionType.CALL:
            return False
        intrinsic_value = max(self.strike_price - self.stock_price,
                              0) if self.option_type == OptionType.PUT else max(self.stock_price - self.strike_price, 0)
    ...
    def sto_after_btc_a_loss(self, option_chains: OptionChains) -> tuple:
        ...
        new_strike_price = self.strike_price * \
            (1 - ROLLOUT_LOSING_TRADE_STRIKE_PRICE_LOWER_PERCENTAGE)
        ...
        new_option_premium = self.option_market_price + \
    ...
    def create_a_rollout_order(self,
        ...
        sto_symbol = Options.form_an_option_symbol(
            self.ticker, new_expiration_date, new_strike_price, self.option_type)
        order_dict = {
    ...
    def sto_an_option_order(ticker: str,
                            ...
                            cost_basis: float = -1.0) -> dict:
        today_date = datetime.today().date()
        expiration_date = today_date + \
            timedelta(days=(4 - today_date.weekday()) %
                      7 + 7*min_expiration_weeks)
        option_candidates = None
        if cost_basis > 0 and option_type == OptionType.CALL:
            option_candidates = option_chains.get_call_option_candidates_from_min_strike_price_and_min_premium_percentage(
                min_strike_price=cost_basis, min_premium_percentage=min_premium_percentage, min_premium=min_premium)
        else:
    ...
    def form_an_option_symbol(ticker: str,
        ...
        formatted_strike_price = ''.join(
            ['0' for i in range(num_zeros)]) + int_strike_price
